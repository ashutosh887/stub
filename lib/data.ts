...
const SPEND_GROUP: Record<LedgerQuery["groupBy"], string | null> = {
  ...
  vendor: "va.name",
  account: "da.name",
  agent: "COALESCE(d.agent_id, '(none)')",
  intent: "COALESCE(d.intent, '(none)')",
  costCenter: "COALESCE(d.cost_center, '(none)')",
  reason: null,
...
const DENIAL_GROUP: Record<LedgerQuery["groupBy"], string | null> = {
  ...
  vendor: null,
  account: "a.name",
  agent: "COALESCE(dn.agent_id, '(none)')",
  intent: "COALESCE(dn.intent, '(none)')",
  costCenter: "COALESCE(dn.cost_center, '(none)')",
  reason: "dn.reason",
